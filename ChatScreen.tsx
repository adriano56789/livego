...
import { BackIcon, ThreeDotsIcon, SendIcon, GalleryIcon, CheckIcon, DoubleCheckIcon, UserIcon, CloseIcon, LiveIndicatorIcon, ClockIcon, WarningTriangleIcon } from './components/icons';
import BlockReportModal from './components/BlockReportModal';
import { useTranslation } from './i18n';
import { api } from './services/api';
import { LoadingSpinner } from './components/Loading';
import { socketService } from './services/socket';
import { createChatKey, avatarFrames, getRemainingDays, getFrameGlowClass } from './utils/chatUtils';
...
const ChatMessageBubble: React.FC<{ message: Message; isMe: boolean; user: User; onImageClick: (url: string) => void; }> = ({ message, isMe, user, onImageClick }) => {
    ...
    const activeFrame = (user.activeFrameId && activeOwnedFrame && remainingDays && remainingDays > 0)
        ...
        : null;
    // Note: AvatarFrame components should be loaded dynamically from API
    const ActiveFrameComponent = null; // Placeholder - implement dynamic loading
