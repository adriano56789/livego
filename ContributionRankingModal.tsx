...
type Period = 'Diária' | 'Semanal' | 'Mensal';
...
const ContributionRankingModal: React.FC<{ onClose: () => void; }> = ({ onClose }) => {
    ...
    useEffect(() => {
        setIsLoading(true);
        const periodMap: Record<Period, PeriodKey> = {
            'Diária': 'daily',
            'Semanal': 'weekly',
            'Mensal': 'monthly',
        };
        api.getRankingForPeriod(periodMap[activeTab])
            ...
            >
                <header className="flex-shrink-0 p-4">
                    ...
                    <nav className="flex items-center justify-center space-x-6">
                        {(['Diária', 'Semanal', 'Mensal'] as Period[]).map(tab => (
