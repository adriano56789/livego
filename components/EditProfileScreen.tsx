...
import { BackIcon, PlusIcon, ChevronRightIcon, TrashIcon } from './icons';
...
const EditProfileScreen: React.FC<EditProfileScreenProps> = ({ user, onBack, onSave }) => {
  ...
  const handleSave = () => {
    ...
    if (finalData.obras && finalData.obras.length > 0) {
        finalData.avatarUrl = finalData.obras[0].url;
    } else {
    ...
    if (e.target.files && e.target.files[0]) {
      ...
      if (currentObras.length < 8) {
        ...
        reader.onload = (event) => {
            ...
            if (dataUrl) {
                const newObra: Obra = { id: `obra-local-${Date.now()}`, url: dataUrl };
        ...
        <div className="py-4">
          <div className="grid grid-cols-4 gap-3">
            {(formData.obras || []).map((obra, index) => (
              ...
              >
                <img src={obra.url} onError={handleImageError} alt={`Profile photo ${index + 1}`} className="w-full h-full object-cover rounded-lg bg-[#2c2c2e]" />
                {index === 0 && (
