...
const FollowersScreen: React.FC<FollowersScreenProps> = ({ currentUser, viewedUserId, onExit, onUpdateUser, onViewProfile }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setIsLoading(true);
            try {
                const followerUsers = await authService.getFollowers(viewedUserId);
    ...
    const renderContent = () => {
        ...
        if (users.length === 0) {
            return <div className="flex-grow flex items-center justify-center text-gray-500">Ninguém está te seguindo ainda.</div>;
        }
        return (
