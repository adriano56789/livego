  ...
  return (
    <div className="absolute inset-0 bg-black z-[100] flex flex-col no-scrollbar" ref={containerRef} style={{ scrollSnapType: 'y mandatory', overflowY: 'scroll' }}>
      ...
      {photos.map((photo) => {
        ...
        return (
          <div key={photo.id} className="w-full h-full flex-shrink-0 relative" style={{ scrollSnapAlign: 'start' }}>
            <img src={photo.photoUrl} alt="Full screen view" className="w-full h-full object-contain" />
            <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/60 to-transparent">
              <div className="flex justify-between items-center">
                <button onClick={() => onViewProfile(photo.user)} className="flex items-center space-x-2">
                  <img src={photo.user.avatarUrl} alt={photo.user.name} className="w-10 h-10 rounded-full object-cover border-2 border-white" />
