  ...
  useEffect(() => {
    const fetchArticle = async () => {
      ...
      try {
        ...
        console.error(error);
        setArticle({ id: 'error', title: 'Erro', content: 'Não foi possível carregar o artigo.' });
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <div className="h-screen w-full bg-black text-white flex flex-col font-sans">
      <header className="p-4 flex items-center justify-between shrink-0 border-b border-gray-800">
        ...
        <h1 className="font-bold text-lg truncate px-4">{isLoading ? 'Carregando...' : article?.title}</h1>
      ...
      <main className="flex-grow p-6 overflow-y-auto">
        {isLoading ? (
          ...
          <div className="text-gray-300 space-y-4" dangerouslySetInnerHTML={{ __html: `
            <h2 class="text-2xl font-bold text-white mb-6">${article.title}</h2>
            ${article.content}
        ` }} />
        ) : null}
      </main>
