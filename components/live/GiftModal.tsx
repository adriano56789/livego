                    ...
                    <nav className="flex items-center space-x-4 border-b border-gray-700">
                        {giftCategories.map(tab => (
                            <button 
                                ...
                                className={`text-sm font-semibold transition-colors relative py-2 ${activeTab === tab ? 'text-white' : 'text-gray-500'}`}
                                disabled={loadingCategories.has(tab)}
                            >
                                {loadingCategories.has(tab) ? 'Carregando...' : tab}
