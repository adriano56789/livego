...
export const FriendController = {
    async sendRequest(req: any, res: any) {
        ...
        // Check if already friends (bi-directional follow)
        const isFollowing = db.following.get(fromUserId)?.has(toUserId);
        const isFan = db.fans.get(fromUserId)?.has(toUserId); // Wait, this logic in user controller was simply follower/following.
        // True "Friend" usually means both follow each other.
