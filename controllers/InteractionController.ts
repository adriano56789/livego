...
export const InteractionController = {
    ...
    async logVisit(req: any, res: any) {
        ...
        if (profileId === visitorId) return res.status(200).json({}); // Don't log self visit

        const visits = db.visits.get(profileId) || [];
        // Prevent duplicate spam logging (simple throttle check could go here)
        const newVisits = [{ visitorId, timestamp: new Date().toISOString() }, ...visits];
        db.visits.set(profileId, newVisits.slice(0, 50)); // Keep last 50
        saveDb();
        return res.status(200).json({ success: true });
    },
