...
export const WalletController = {
  ...
  async getEarningsInfo(req: any, res: any) {
    ...
    const policy = db.earningsPolicy;
    const available_diamonds = user.earnings;
    
    // Use dynamic policy from DB
    const conversionRate = policy.conversionRate / 10000; // Example math, adjusting to existing logic (100 = 0.01)
    const gross_brl = truncateBRL(available_diamonds * 0.01); 
    const platform_fee = truncateBRL(gross_brl * (policy.platformFeePercentage / 100));
    ...
    return res.status(200).json({
        ...
        net_brl,
        policy // Return policy metadata if needed by frontend
    });
  },
  ...
  async calculateWithdrawal(req: any, res: any) {
      ...
      const fee = truncateBRL(gross * (policy.platformFeePercentage / 100));
  ...
  async withdrawEarnings(req: any, res: any) {
      ...
      if (amount < policy.minimumWithdrawalAmount) return res.status(400).json({ error: `Minimum withdrawal is ${policy.minimumWithdrawalAmount}` });
  ...
  // ... (Other methods createOrder, confirmPurchase, buyDiamonds, getHistory remain similar)
  ...
  async getHistory(req: any, res: any) {
      const { userId } = req.params;
      const history = db.purchases.filter((p: any) => p.userId === userId)
