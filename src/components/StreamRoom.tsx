...
import { Streamer, User, Gift, ToastType, RankedUser, LiveSessionState, UserPreferences } from '../types';
...
// Fix: Added missing PKBattleScreen import
...
const defaultPreferences = {
    ...
    privacy: { showLocation: true, showActivityStatus: true, chatPermission: 'all' as const },
    ...
    stream: {
        beauty: {},
        private: { privateInvite: true, followersOnly: true, fansOnly: false, friendsOnly: false }
    },
    permissions: { camera: 'prompt' as const, microphone: 'prompt' as const }
};
...
// Define type for swipe coordinates to avoid inline generic parser issues
type SwipeCoords = { x: number; y: number };
...
const StreamRoom: React.FC<StreamRoomProps> = ({ streamer, onRequestEndStream, onLeaveStreamView, onStartPKBattle, onViewProfile, currentUser, onOpenWallet, onFollowUser, onOpenPrivateChat, onOpenPrivateInviteModal, setActiveScreen, onStartChatWithStreamer, onOpenPKTimerSettings, onOpenFans, onOpenFriendRequests, gifts, receivedGifts, updateUser, liveSession, updateLiveSession, logLiveEvent, onStreamUpdate, refreshStreamRoomData, addToast, followingUsers, streamers, onSelectStream, onOpenVIPCenter }) => {
    ...
    const swipeStart = useRef<SwipeCoords | null>(null);
